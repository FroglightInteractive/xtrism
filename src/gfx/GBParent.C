...
void GBParent::losechild(GBox *ch) {
  ...
  for (list<GBChildInfo>::iterator i=children.begin(); i!=children.end(); ++i)
    (*i).undepend(*chi);
  children.erase(chi);
...
void GBParent::placechildren_(bool vnoth) {
  ...
  while (!allplaced)
    { bool chainplaced=false;
      ...
      firsttime=false;
      noneplaced=true; allplaced=true;
      for (i=children.begin(); i!=children.end(); ++i)
        { if (!(*i).placed(vnoth))
  ...
  if (ch.double_depend(vnoth))
    { int min, max;
      if (!pc_get_minmax(vnoth, GBP_Min, ch, min) ||
        ...
        athrow("GBParent::pc_one: Couldn't determine position for a doubly connected child");
...
bool GBParent::pc_get_minmax(bool vnoth, bool side,
  ...
  if (ch.whence(vnoth,side) == this)
    { res=ch.edge(vnoth,side,side); return true; }
  else if (ch.edge_fixed(vnoth,side,this))
    { res=ch.edge(vnoth,side,side ^ (ch.dist(vnoth,side)>=0)); return true; }
  return false;
  }
