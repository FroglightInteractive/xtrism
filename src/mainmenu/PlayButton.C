...
bool PlayButton::key(int kc, bool io) {
  ...
  switch (tenv().lastchar())
    {
    case 13: if (isselected()) { play(); return true; }
      break;
    case 's': if (!isselected()) { visualselect(true); takeselect(); }
      break;
    case '1': if (isselected()) { play(0); return true; }
      break;
    case '2': if (isselected()) { play(1); return true; }
      break;
    case '3': if (isselected()) { play(2); return true; }
      break;
    case '4': if (isselected()) { play(3); return true; }
      break;
    }
  return false;
  }
