...
bool WishList::logkey(BufferCode b) {
  ...
  switch (b)
    {
    case BC_iLeft: movemark.left=true; return true;
    case BC_oLeft: movemark.zleft&=2; movemark.left=false; return true;
    case BC_oZLeft:
      if (movemark.zleft==3)
        movemark.left=true;
      movemark.zleft&=2; return true;
    case BC_iZLeft: movemark.zleft=3; return true;
    case BC_iRight: movemark.right=true; return true;
    case BC_oRight: movemark.zright&=2; movemark.right=false; return true;
    case BC_oZRight:
      if (movemark.zright==3)
        movemark.right=true;
      movemark.zright&=2; return true;
    case BC_iZRight: movemark.zright=3; return true;
    case BC_iRotCW: movemark.rotcw=true; return true;
    case BC_iRotCCW: movemark.rotccw=true; return true;
    case BC_iDrop: movemark.drop=true; return true;
    default: return false; // not handling any of the others
    }
  }

void WishList::clearflags() {
  movemark.left=movemark.right=movemark.zleft=movemark.zright=
    movemark.rotcw=movemark.rotccw=movemark.drop=0;
