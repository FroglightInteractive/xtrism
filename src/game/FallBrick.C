...
BEClash FallBrick::newbrick(int bno) {
  pos.bno=bno; pos.rot=rot0;
  pos.x=x0; pos.y=y0; // correction for certain bricks?
...
FBHResult FallBrick::movex(int dir, bool zap)  {
  ...
  do {
    ...
    case BEC_OK:
      ...
      res.moved=1; pos.x+=dir;
      break;
    case BEC_Brick:
      if (tryshoosh(dir)) {
	willing_to_swap=false;
	res.moved=1; res.shooshed=1;
      } else if (env->offerswap(*this)) {
	willing_to_swap=false;
	res.moved=1; res.shooshed=1; res.othermoved=1;
      } else {
	res.hitother=1;
...
FBHResult FallBrick::rotate(int dir) {
  ...
  for (unsigned int i=0; i<sizeof(moveforrotl)/sizeof(*moveforrotl); i++)
