...
NiceGame::NiceGame(Session &s, int pos0,
		   ...
		   BrickSprites const &bs0, BrickSprites const &bs1,
                   ...
                   class Logger *logger):
  GBParent(&s.gbpar(), s.gbpar().width()/(pos0?2:1),s.gbpar().height()),
...
void NiceGame::bricklanded(FBPos const &pos, PlPlayer *plp) {
  ...
  puddreq=plp;
  pudding=1;
  int i=GameTime::PRE_PUD; pudtime.reset(i); sendreq(pudtime.getnext());
  pudlns=0;
  }

void NiceGame::poll() {
  if (pudding && pudtime.ivalgone())
    { int i=GameTime::MID_PUD; pudtime.adjust(i); sendreq(pudtime.getnext());
      int y=logpit->findfullline();
      if (y>=0)
        { logpit->collapseline(y,pudding);
          vispit->pudding(y,pudding); sounds->explode();
          lines++;
