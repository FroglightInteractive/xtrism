  ...
  for (int c=0; c<brk.cells(); c++)
    { int x=pos.x+brk.x(c) - 4; // experimental !!!!!
      ...
      if (x>=0 && x<wid && y>=0 && y<hei)
        { refcell(x,y)=addnotrem?&(usesec?bsp2:bsp).cell(pos.bno,pos.rot,c) : 0;
          setchanged(x,y);
        }
      else
...
void VisPit::copyline(int dst, int src) {
  for (int x=0; x<wid; x++)
    if (cell(x,dst)!=cell(x,src))
      { refcell(x,dst)=cell(x,src); setchanged(x,dst); }
  }

void VisPit::zeroline(int dst) {
  for (int x=0; x<wid; x++)
    if (cell(x,dst)!=0)
      { refcell(x,dst)=0; setchanged(x,dst); }
  }
    ...
    if (op_inuse[i])
      { FBPos &p=oldpos[i];
        if ((p.y+1<y && dir>0) || (p.y+2>y && dir<0))
          { p.y+=dir; /* addbrick(p,i,0); */ } // EXPERIMENTAL !!!
      }
  sendwarn();
  }
...
void VisPit::addbrick(FBPos const &pos, int id, bool definitive) {
  ...
  if (id<0)
    { addrem(pos,true); sendwarn(); return; }
  ...
  if (!definitive)
    { op_inuse[id]=true; oldpos[id]=pos; }
  sendwarn();
  }
