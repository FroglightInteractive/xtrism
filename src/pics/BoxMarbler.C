    ...
    { float Y=float(y)/tileh;
      for (unsigned int x=0; x<tilew; x++)
        { float X=float(x)/tilew;
          tile->c(x,y)=byte(Y*(X*bc.cc(x,y) + (1-X)*bc.cc(x+tilew,y)) +
...
BoxMarbler::BoxMarbler(TRGB const &rgb0, BoxMarblers *par):
  ...
  WideRGB rgb0a(rgb); rgb0a-=100;
  WideRGB rgb1(rgb); rgb1+=100;
...
void BoxMarbler::draw(BBox const &mybbox, BBox const &redrawbbox,
  ...
  BBox interior(mybbox); interior.addmargin(-borderw);
  interior&=redrawbbox;
  img->tile(interior);
  ...
  for (int y=0; y<borderw; y++)
    { for (int x=y; x<width-y-1; x++)
      ...
      bb&=redrawbbox;
      if (bb)
        { Point pp(bb.topleft()); bb.shift(0,-top-y);
  ...
  for (int y=0; y<borderw; y++)
    { for (int x=y+1; x<width-y; x++)
        { int p=parent->readpoint(x+left,bottom-1-y)-128+BOTTOMRAISE*raise;
          TRGB t(rgb); t+=p;
          parent->horiborder->putpix(left+x,0,cm(t));
        }
      ...
      bb&=redrawbbox;
      if (bb)
        { Point pp(bb.topleft()); bb.shift(0,-bottom+1+y);
          parent->horiborder->partput(bb,pp);
        }
    }
  ...
  for (int x=0; x<borderw; x++)
    { for (int y=x+1; y<height-x; y++)
      ...
      bb&=redrawbbox;
      if (bb)
        { Point pp(bb.topleft()); bb.shift(-left-x,0);
  ...
  for (int x=0; x<borderw; x++)
    { for (int y=x; y<height-1-x; y++)
        { int p=parent->readpoint(right-1-x,top+y)-128+RIGHTRAISE*raise;
          TRGB t(rgb); t+=p;
          parent->vertborder->putpix(0,top+y,cm(t));
        }
      ...
      bb&=redrawbbox;
      if (bb)
        { Point pp(bb.topleft()); bb.shift(-right+1+x,0);
          parent->vertborder->partput(bb,pp);
        }
    }
  }
