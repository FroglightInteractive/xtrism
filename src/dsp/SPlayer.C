...
void SPlayer::activate() throw (SampleExc) {
  ...
  if (ioctl(fd, SNDCTL_DSP_GETFMTS, &mask) < 0) {
    perror("SPlayer: SNDCTL_DSP_GETFMTS");
    throw SampleExc();
  }
  if (mask & AFMT_S16_LE) {
    int fmt = AFMT_S16_LE;
    if (ioctl(fd, SNDCTL_DSP_SETFMT, &fmt) < 0) {
      perror("SPlayer: SNDCTL_DSP_SETFMT");
      throw SampleExc();
    }
    p = &SPlayer::polls16;
    bytespsam = 2;
  ...
  if (ioctl(fd, SNDCTL_DSP_STEREO, &s) < 0) {
    perror("SPlayer: SNDCTL_DSP_STEREO");
    throw SampleExc();
  }
  stereo = (s == 1);
  ...
  if (ioctl(fd, SNDCTL_DSP_SPEED, &spd) < 0) {
    perror("SPlayer: SNDCTL_DSP_SPEED");
    throw SampleExc();
  }
  pfreq = spd;
  ...
  if (ioctl(fd, SNDCTL_DSP_SETFRAGMENT, &arg) < 0) {
    perror("SPlayer: SNDCTL_DSP_SETFRAGMENT");
    throw SampleExc();
  }
  audio_buf_info info;
  if (ioctl(fd, SNDCTL_DSP_GETOSPACE, &info) < 0) {
    perror("SPlayer: SNDCTL_DSP_GETOSPACE");
    throw SampleExc();
  }
  int nfrags
    = max(int((bytespsec * SP_MAXAHEAD) / fragsize), 2); // # frags I want to use
  freefrags = info.fragstotal - nfrags; // leave rest free
  dbx(-980808, "SPlayer: nfrags=%i, info.fragstotal=%i, info.fragments=%i",
      nfrags, info.fragstotal, info.fragments);
  dbx(-980808, "SPlayer: fragsize=%i, info.fragsize=%i info.bytes=%i",
      fragsize, info.fragsize, info.bytes);
