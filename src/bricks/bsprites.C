...
BrickSprites::BrickSprites(const SBrickData &sbd, const Filename &basename,
  ...
  char buf[10];
  sprintf(buf,"%i",size); base.addpart(buf);
  ...
  for (unsigned int bno=0; bno<nn; bno++) {
	...
	break;
      ...
      rgb1=rgb0; rgb1.r+=100; rgb1.g+=100; rgb1.b+=100;
      rgb0.r-=100; rgb0.g-=100; rgb0.b-=100;
    }
    switch (rbd.rotstyle()) {
    case BrickData::ROT_4x4: case BrickData::ROT_4x4_TWO:
	...
	rotd=0;
      ...
      break;
    default: athrow("BrickSprites: undefined rotation style");
    }
    if (mother)
      delete mother;
    mother=0;
...
BrickCell *BrickSprites::docreate(int cel,
  ...
  switch (rot&3)
    ...
    case 1: x=maxxy-y0; y=x0; break;
    case 2: x=maxxy-x0; y=maxxy-y0; break;
    case 3: x=y0; y=maxxy-x0; break;
    }
  BrickData const &bd=rbd[rot];
  BrickCell *bc=new BrickCell(*mother,size*x, size*y, size);
  x=bd.x(cel); y=bd.y(cel);
