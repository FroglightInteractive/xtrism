...
void TCmap::staticinit(int rpos, int rbits, int gpos, int gbits,
  ...
  rright = 8 - rbits;
  rleft = rpos;
  rmask = (255 >> rright) << rpos;
  radd = 127 >> rbits;
  gright = 8 - gbits;
  gleft = gpos;
  gmask = (255 >> gright) << gpos;
  gadd = 127 >> gbits;
  bright = 8 - bbits;
  bleft = bpos;
  bmask = (255 >> bright) << bpos;
  badd = 127 >> bbits;
  greybits = leftbits;
}
  ...
  if (tabulate) {
    for (int b = 0; b < 16; b++)
      for (int g = 0; g < 16; g++)
        for (int r = 0; r < 16; r++)
          table[r + 16 * g + 16 * 16
                * b] = nearest(r, g, b, ngrey, nr, ng, nb);
  }
...
TCol TCmap::nearest(int r, int g, int b, int ngrey, int nr, int ng, int nb) {
  ...
  g -= (green * 16) / ng;
  b -= (blue * 16) / nb;
  int grey = 2 * r * 256 * nr / 16 + 3 * g * 256 * ng / 16 + b * 256 * nb
             / 16;
...
#else
  int classes[] = {
    TrueColor, PseudoColor, DirectColor, GrayScale,
    StaticColor, StaticGray
  };
  if (depth <= 8) {
    classes[0] = PseudoColor;
    classes[1] = TrueColor;
  }
#endif
