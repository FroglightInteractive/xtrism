...
TFont::TFont(const TMFont &tmf, const TRGB &fg, const TRGB &bg):
  ...
  gc = XCreateGC(tgfx.disp(),
                 tgfx.win(), GCForeground | GCBackground | GCFont, &gcv);
...
void TFont::write(char const *string, int x, int y,
  ...
  y += ycor;
  if (bg) {
    XDrawImageString(tgfx.disp(), tgfx.win(), gc, x, y, string,
                     strlen(string));
    int dw = totalw - sw;
    if (dw > 0)
      switch (align) {
      ...
      case CENTRE:
        clearrect(x + sw, y, dw / 2);
        clearrect(x - dw / 2, y, dw / 2);
        break;
      }
  } else
