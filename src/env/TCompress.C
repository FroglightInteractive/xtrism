...
TCompress::TCompress(Filename const &fn) {
  ...
  close(pfd[0]); close(fd); // those are for child
...
TDecompress::TDecompress(Filename const &fn):
  ...
  tthrow(fd<0,string("TDecompress: failed to open `")+fn.name()+"': "
         +strerror(errno));
  int pfd[2];
  tthrow(::pipe(pfd),string("TDecompress: pipe() failed: ")+strerror(errno));
  ...
  tthrow(fres<0,string("TCompress: fork() failed: ")+strerror(errno));
  close(pfd[1]); close(fd); // those are for child
...
TDecompress::~TDecompress() {
  ...
  tthrow(WEXITSTATUS(stt),"~TDecompress: decompression failed: child exited with non-zero code\nIf this was a file in .../cache/, delete it and try to rerun trism.");
  }
...
int TDecompress::readln(char *dst) {
  ...
  while (1)
    { if (!usedbuflen)
      ...
      byte *nextnl=usedbuflen?reinterpret_cast<byte*>(memchr(buffer+bufstartoff,'\n',usedbuflen))
                             :buffer+bufstartoff;
      if (nextnl)
        { int thislen=nextnl-(buffer+bufstartoff);
          ...
          totallen+=thislen;
          dst[thislen]=0;
          bufstartoff+=thislen+1; usedbuflen-=thislen+1;
          return totallen;
        }
