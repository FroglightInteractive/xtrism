...
TImage::TImage(const TEnv &tenv, unsigned int w, unsigned int h):
  ...
  dbx(3,"TIm: depth=%i, bpp=%i, sp=%i bpl=%i",env.depth(),env.bpp(),sp,bpl);
...
unsigned int TImage::width() const { return image->width; }
unsigned int TImage::height() const { return image->height; }
unsigned int TImage::depth() const { return image->depth; }
...
void TImage::tile(BBox const &dest) const {
  ...
  for (int y=y0; y<dest.bottom(); y+=h)
    for (int x=x0; x<dest.right(); x+=w)
      { BBox b(x,y,x+w,y+h); b&=dest; // destination rectangle
        BBox c(b); c.shift(-x,-y); // source rectangle
        partput(c,b.topleft());
      }
  }
